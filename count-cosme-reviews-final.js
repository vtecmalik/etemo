...
async function countCosmeReviews() {
  console.log('🔍 Анализ отзывов для cosme_products с баркодами...\n');

  try {
    // 1. Загружаем ВСЕ продукты с баркодами из cosme_products
    ...
    console.log('   (это может занять несколько минут, отзывов очень много)\n');

    let allReviews = [];
    page = 0;
    hasMore = true;
    let maxPages = 500; // Ограничение чтобы не ждать вечно (500k отзывов)
    ...
    while (hasMore && page < maxPages) {
      ...
      if (error) {
        console.log(`⚠️  Достигнут лимит/таймаут на странице ${page}. Продолжаем с загруженными данными...`);
        break;
      }

      if (data && data.length > 0) {
        allReviews = allReviews.concat(data);
        if (page % 10 === 0) { // Выводим прогресс каждые 10k отзывов
    ...
    console.log('\n' + '═'.repeat(70));
    console.log('📊 СТАТИСТИКА ПО ОТЗЫВАМ НА ПРОДУКТЫ С БАРКОДАМИ (cosme_products)');
    console.log('═'.repeat(70));
    console.log('');
    console.log(`📦 Продуктов с баркодами в cosme_products: ${allProductsWithBarcodes.length.toLocaleString('ru-RU')}`);
    ...
    // Общая статистика
    console.log('📊 ОБЩАЯ СТАТИСТИКА (все отзывы в базе):');
    console.log('─'.repeat(70));
    ...
    if (page >= maxPages) {
      console.log(`   ⚠️  Загружено только первые ${maxPages * pageSize} отзывов (лимит)`);
    }
    console.log('─'.repeat(70));
    console.log('');
    console.log('═'.repeat(70));
